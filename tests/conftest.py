...
@pytest.fixture(autouse=True)
def _no_background_join_reconciler(monkeypatch):
    """Never let the join reconciler's DAEMON THREAD outlive the test that started it.

    `maybe_reconcile_join` spawns a daemon thread that polls the DB every 5s for up to 24
    attempts — two minutes. A unit test finishes in milliseconds, so that thread reliably
    outlives it: `migrated_home` then disposes the engine, clears the `get_engine` /
    `get_settings` lru_caches and lets pytest delete the temp home, while the thread is still
    calling `store.list_for_run(...)` against it. The thread rebuilds and re-caches an engine
    from whatever `XORCISE_HOME` happens to be at that instant, and the NEXT test inherits it —
    which is why the failure lands as `no such table: runs` on an unrelated test's INSERT, and
    why the victim moves between neighbouring tests run to run.

    It is timing-dependent, so it is rare locally and common under `-n auto` on a CI runner with
    a different core count. Observed on GitHub Actions twice, on two different tests in the same
    file, on a tree that passed here.

    No coverage is lost. This stubs only the KICKER — the reconcile logic itself
    (`reconcile_join` / `reconcile_telemetry`) is exercised directly and synchronously with an
    injected store in `tests/unit/test_join_reconcile.py`. Tests that assert the kick happened
    monkeypatch this same attribute themselves; theirs is applied later, so it wins. The routers
    import the symbol INSIDE the function body, so patching the module attribute reaches every
    call site.
    """
    monkeypatch.setattr(
        "xorcise.core.rest.join_reconcile.maybe_reconcile_join",
        lambda run_id: None,
    )
