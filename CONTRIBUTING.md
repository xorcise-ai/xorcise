...
| `integration` | **yes** | no | ✅ any PR that changes more than docs |
| `e2e` | **yes** + fixed ports | no | ✅ any PR that changes more than docs |
...
a documentation-only change that should still run them anyway — a rewrite of this file's
runner instructions, say. (The documentation site lives in its own repository; there is no
`docs/` directory here.)

The label takes effect **on the next push**, not immediately: `ci.yml` deliberately does not
trigger on label events, because a release-note label is mandatory on every pull request and
re-running the whole suite each time one lands would supersede an already-green run. So add
the label, then push — an empty commit is enough.
...
Two things about a pull request are checked by CI, because both outlive the merge. They are
checked by the `pr-contract` workflow, which reports a status check of the same name.

> **Maintainers:** that context has to be **registered as a required status check** on `main`
> before it blocks anything. Until it is, `pr-contract` reports and is advisory — the `main`
> ruleset currently requires `ci-ok` only. Register the context exactly as the job names
> itself: `pr-contract`.

**The title must be a Conventional Commits subject** — `<type>(<optional scope>)!: <subject>`.
The type and any scope are lowercase; a scope may list more than one component
(`feat(cli,rest):`). Pull requests here are normally **squash-merged**, and the title is what
GitHub offers as the squash subject, so in practice it usually becomes the commit message on
`main`, where a malformed one cannot be corrected without rewriting history. Write it as the
...
"Usually", not "always": `main` still permits merge and rebase, and a single-commit pull
request squashes under the *commit* subject rather than the title. Treat the gate as a strong
convention, not a guarantee about the bytes that land. A `Revert "…"` title generated by
GitHub's Revert button is accepted as-is, so a UI revert needs no retitle.
